...
description: "Discover failure modes in LLM traces or agent logs before writing metrics. Use when inspecting traces, finding bugs in AI outputs, or starting an eval dataset. HTML review app, sequential scanning, subagent compatible."
---

# Error Discovery

Discover failure modes in AI applications before writing metrics.
Sequential workflow, subagent compatible, 0 extra dependencies.
...
3. Start the helper server in the background:
   ```bash
    python3 ~/.config/opencode/skills/error-discovery/scripts/server.py 8080 &
   ```
