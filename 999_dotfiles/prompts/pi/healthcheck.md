---
description: Run sysinfo.sh --tech and review ~/sysinfo.log for system health
---
...
1. Run system info collection:
   ...
   - Output goes to `~/sysinfo.log` (script redirects stdout).

2. Read `~/sysinfo.log` completely (use `read` tool, handle ANSI colors).

3. Analyze each section and make health table:

   | Area | Status | Value | Action |
   |------|--------|-------|--------|

   Check these thresholds (lean box: 6.3 Gi RAM, 31G disk):

   - **Disk:** Use% <80% = OK, 80-90% = WARN (clean), >90% = CRITICAL. Root is priority. Current baseline ~67% (9.9G free).
   ...
   - **Tech stack (--tech):** All must be OK:
     - Java Temurin-25 at /usr/lib/jvm/temurin-25-jdk-amd64
     - Clojure CLI 1.12.6, Lein 2.12.0, bb 1.13.219, clj-kondo, clojure-lsp, neil, jet, cljfmt
     ...
     - Gemini Antigravity: check paths only, never log tokens
