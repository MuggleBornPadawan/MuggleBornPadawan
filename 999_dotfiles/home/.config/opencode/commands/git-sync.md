---
description: Pi prompt — add, commit, push branch, merge into main
---
...
2. If already on the default branch (`main` or `master`):
   ...
   - Commit with the message below or generate a conventional commit message.
   - Push to remote.
   - Stop here.
3. Check `git status` and `git diff`.
4. Stage all changes (`git add .`).
5. Commit the changes:
   - Use this commit message if provided: `$ARGUMENTS`
...
> Source: `~/.pi/agent/prompts/git-sync.md`
