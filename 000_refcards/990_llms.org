...
- [[#getting-started][getting started]]
  - [[#resources][resources]]
  - [[#llm-selection-and-benchmarking][llm selection and benchmarking]]
  - [[#prompting-and-agent-cookbooks][prompting and agent cookbooks]]
  - [[#prompt-tools][prompt tools]]
  - [[#temperature-top-p-and-top-k][temperature, top-p and top-k]]
  - [[#new-project][new project]]
  - [[#smaller-pull-requests-prs][smaller pull requests (prs)]]
  - [[#front-end-pull-request-pr][front end pull request (pr)]]
  - [[#recommendation-check][recommendation check]]
  - [[#cost-audit][cost audit]]
  - [[#gauntlet-loop][gauntlet loop]]
  - [[#loop-engineeering][loop engineeering]]
  - [[#asd-ste100-simplified-technical-english-ste][ASD-STE100 Simplified Technical English (STE)]]
  - [[#hybrid-architecture-of-agents][hybrid architecture of agents]]
  - [[#red-blue-green-review-loop][red blue green review loop]]
  - [[#build-your-first-agent][build your first agent]]
  - [[#cto-as-a-prompt][cto as a prompt]]
  - [[#audit-a-merge-request][audit a merge request]]
  - [[#ltx---direct-a-shot-not-screenplay][ltx - direct a shot; not screenplay]]
  - [[#cool-website-ui-build-in-stages-not-vibes][cool website ui: build in stages, not vibes]]
  - [[#b2b-case-study-engine][b2b case study engine]]
  - [[#operations-consultant][operations consultant]]
  - [[#find-your-unknowns][find your unknowns]]
  - [[#fan-out-method][fan out method]]
  - [[#production-check][production check]]
  - [[#for-beginners][for beginners]]
  - [[#medical-appointment][medical appointment]]
  - [[#loop-engineer][loop engineer]]
  - [[#gguf][gguf]]
  - [[#record-a-skill][record a skill]]
  - [[#permission-brief-agent][permission brief agent]]
  - [[#workflow-for-messy-tasks][workflow for messy tasks]]
  - [[#task-router][task router]]
  - [[#security-gut-check][security gut check]]
  - [[#building-skills][building skills]]
  - [[#elon-musk---five-step-rubric][elon musk - five step rubric]]
  - [[#grill-me---spec-to-code][grill me - spec to code]]
  - [[#coding-agent-instructions][coding agent instructions]]
  - [[#stop-being-a-yes-man][stop being a YES man]]
  - [[#llm-coding-problems-and-solution][llm coding problems and solution]]
  - [[#the-llm-council---andrej-karpathy][the llm council - andrej karpathy]]
  - [[#power-users][power users]]
  - [[#framing][framing]]
  - [[#agents-should-interview-you][agents should interview you]]
  - [[#skills][skills]]
  - [[#prompt-guy][prompt guy]]
  - [[#simple-prompts-to-get-started][simple prompts to get started]]
  - [[#breakdown-spec][breakdown spec]]
  - [[#claudmd][claud.md]]
  - [[#openclaw-3-layer-memory-system][openclaw 3-layer memory system]]
  - [[#notebookllm][notebookllm]]
  - [[#consulting][consulting]]
  - [[#grumpy-senior-dev][grumpy senior dev]]
  - [[#assistant][assistant]]
  - [[#aiml-tutor][ai/ml tutor]]
  - [[#professional-headshot][professional headshot]]
  - [[#absolute-beast-mode][absolute beast mode]]
  - [[#research-summary][research summary]]
  - [[#crit-method][crit method]]
  - [[#unbiased-query-request][unbiased query request]]
  - [[#claude][claude]]
  - [[#model-fit][model fit]]
  - [[#chain-of-drafts][chain of drafts]]
  - [[#brian-roemmele-deep-truth-mode][brian roemmele deep truth mode]]
  - [[#cold-start][cold start]]
  - [[#clojure---rich-hickey---simple-made-easy---hacks][clojure - rich hickey - simple made easy - hacks]]
- [[#custom-instructions][custom instructions]]
  ...
  - [[#tone-and-persona-perspectives][tone and persona perspectives]]
  - [[#sound-more-human][sound more human]]
  - [[#isaac-asimov-way-of-writing][isaac asimov way of writing]]
  - [[#cornell-style-numbering][cornell-style numbering]]
  - [[#art-world][art world]]
...
|--------------------------------------------------+-----------------------+---------------|
** new project
- /grill-me i want to start a new project
- document the requirements
- license version
- create a plan
- create pre build checklist
- create test cases with test data (including edge cases)
- /btw what is happening? 
- start building sequentially phase by phase. show me real time progress. store progress status in a local file so that you can restart building from that point if interupted. start building ONLY [phase x] now.
- readme file creation
- gh integration
** smaller pull requests (prs)
...
** loop engineeering
Design the Graph, Not the Prompt
...
- ASD-STE100 Simplified Technical English (STE) is a controlled natural language that is designed to simplify and clarify technical documentation
- Always talk in ASD-STE100 Simplified Technical English. Always talk to me like I have ADHD. 
- use in claude.md file 
** hybrid architecture of agents
...
After I approve the Agent Brief, walk me through setting it up in [ChatGPT / Claude / OTHER TOOL] without requiring code.

** cto as a prompt
I’m building [describe the product] for [describe the customer]. Users will access it through [web, mobile, API, etc.], and it will handle [describe the data]. I currently have [prototype, early users, paying customers, etc.]. Help me plan the AWS architecture, security, authentication, observability, deployment, and cost considerations I should address at this stage. Separate what I need now from what can wait until later.
** audit a merge request
...
** ltx - direct a shot; not screenplay
Build a single shot.
...
** cool website ui: build in stages, not vibes
- seed, crack, roots, beams, rooms, city, reset
...
** b2b case study engine
You are a B2B case study writer. I'll paste raw notes or a transcript from a conversation with a happy customer. Turn it into a publishable case study.

Here is the raw material:
"""
[PASTE YOUR CALL TRANSCRIPT OR NOTES HERE]
"""

Additional context:
- My company/product: [WHAT YOU SELL]
- The customer: [COMPANY NAME + WHAT THEY DO]
- Where this will be published: [website / sales deck / one-pager]

Produce the following:

1. HEADLINE: a specific, results-driven title (include a number if one exists in the notes; never invent metrics).

2. AT-A-GLANCE BOX: Customer, Industry, Company size, and the single biggest result, in 4 short lines.

3. THE STORY in 3 sections:
   - The Challenge (what was broken before us, in the customer's own words where possible)
   - The Solution (what they did with us, concrete, not marketing fluff)
   - The Results (outcomes; use exact numbers from the notes, and flag any claim I should verify before publishing)

4. PULL QUOTE: the single most compelling verbatim line from the customer, lightly cleaned for grammar only.

5. A 2-SENTENCE SUMMARY I can paste into a sales email or LinkedIn post.

Rules:
- Use ONLY facts present in the notes. If a section is thin, tell me exactly what follow-up question to ask the customer to fill the gap. Do not fabricate.
- Keep it tight and skimmable. No jargon, no hype adjectives.
...
if you continue after that, keep an "implementation notes" section where you log everything
** fan out method
- create 5 different versions of a linkedin post summarizing the most recent [newsletter / content]. make 2 of them long posts, 2 short posts, and 1 post optimized to do well on linkedin based on best practices. approach it from different angles and try different styles to get some variety.
...
** for beginners
- prompt/s (hungry kya?)  > project/s (kitchen) > connector/s (grocery shop) > skill/s (recipe) > scheduler/s (oven) > agent/s (chef) 
- agent skills - https://agentskills.io/home 
- skill creator - https://github.com/anthropics/skills/tree/main/skills/skill-creator
  - now can you use your skill creator skill to turn this conversation into a skill I can call anytime I need to [purpose and approach description] and create [goal/s]?
- skills vs agents - a skill is the organizing principle: all the files, instructions, and tools the agent needs to accomplish a specific job. the agent is the thing that reasons and acts. skills are the recipe; agents are the cook
** medical appointment
...
** gguf
- gguf (ggml universal file) is the standard file format for running large language models (llms) locally on your own hardware. developed by georgi gerganov, it packages a model's weights, tokenizer vocabulary, and metadata into a single highly optimized file
** record a skill
...
** elon musk - five step rubric
aim: to figure out what a process needs before trying to make it faster or handing off any part of it to a machine
...
- Gemini instruction: Analyze the provided input by first synthesizing its most robust "Steel Man" abstraction to define its maximum potential; then, conduct a rigorous pressure test to identify existential structural flaws and invalidated assumptions that threaten core viability over surface-level aesthetics. Formulate the single most difficult question that must be resolved to decouple success from failure, and propose one specific, high-impact architectural pivot that maximizes the probability of successful implementation while ruthlessly eliminating unnecessary complexity.
- I want you to act as a sharp, experienced critic who genuinely wants this to succeed — which is exactly why you're going to be honest about what's wrong with it.
- Here is [my idea / plan / piece of content / decision]:
- [PASTE YOUR THING HERE]
- Do this in order:
- Step 1 — Steel Man It In 2-3 sentences, articulate the strongest possible version of this. What's the best case scenario if this works exactly as intended?
- Step 2 — Find The Real Problems Now tell me what's actually wrong with it. Not small edits. Real problems. The assumption that doesn't hold. The audience that isn't clear. The thing I haven't thought about that could make this fail. Be specific. Don't soften it.
- Step 3 — The Hardest Question Ask me the single question I most need to answer before moving forward with this. The one I'm probably avoiding.
- Step 4 — What I'd Change If you were building this instead of me, what would you do differently? One concrete suggestion.
...
* custom instructions
** iq and eq
your iq score is in the range of 120 to 130. your eq score is in the range of 85th to 95th percentile in mayer-salovey-caruso emotional intelligence test. 
