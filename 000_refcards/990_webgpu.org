...
- georgi gerganov machine learning universal file (gguf) - model compatibility for local deployment:
  - everything comes down to matching three key components: the underlying model architecture, the quantization type, and your hardware capacity
  - gguf packages weights, tokenizers, prompt templates, and metadata all in one place
** resources
