#+title: apis
* general information
- endpoint, http methods, requeest-response, status codes, authentication, authorization, access tokens, oauth 2.0, rate limiting, throttling, pagination, caching, idempotency, webhooks, api versioning, openapi, rest vs graphql, api gateway, microservices, error handling 
...
- use oauth2/oidc or api keys
- enforce rbac/abac
- block abuse with rate limits
- sanitize input to stop injections
- log key metadata
- run fuzz tests and stress tests
- update libraries often
- rotate and revoke tokens
- add hsts, csp
- centralize auth and rate limits
- mask or redact sensitive fields
(https://x.com/systemdesignone/status/2026649794274304285?s=20)
#+begin_src js
